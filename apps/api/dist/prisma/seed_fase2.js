...
async function main() {
    ...
    const arroz = await prisma.product.upsert({
        ...
        update: {},
        create: { tenantId: tenant.id, categoryId: mercearia.id, name: "Arroz 5kg", sku: "ARZ5KG", barcode: "7890000000012", unit: client_1.Unit.UN, price: "21.90" }
    });
    const refri = await prisma.product.upsert({
        where: { tenantId_sku: { tenantId: tenant.id, sku: "REF2L" } },
        update: {},
        create: { tenantId: tenant.id, categoryId: bebidas.id, name: "Refrigerante 2L", sku: "REF2L", barcode: "7890000000029", unit: client_1.Unit.LT, price: "9.99" }
    });
    const sabonete = await prisma.product.upsert({
        where: { tenantId_sku: { tenantId: tenant.id, sku: "SBN" } },
        update: {},
        create: { tenantId: tenant.id, categoryId: higiene.id, name: "Sabonete", sku: "SBN", unit: client_1.Unit.UN, price: "3.50" }
    });
